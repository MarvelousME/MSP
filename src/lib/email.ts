...
class EmailService {
  private defaultFrom = process.env.RESEND_FROM_EMAIL || 'My Stable Prime <noreply@mystableprime.com>';
  ...
  private generateWelcomeEmailHTML(data: WelcomeEmailData): string {
    ...
    <head>
      <meta charset="utf-8">
      <title>Welcome to My Stable Prime</title>
      <style>
    ...
    <body>
      <div class="header">
        <h1>Welcome to My Stable Prime! 🎉</h1>
      </div>
      <div class="content">
        ...
        <p>Best regards,<br>The My Stable Prime Team</p>
      </div>
      <div class="footer">
  ...
  private generateReferralNotificationHTML(data: ReferralNotificationData, _symbol?: string): string {
    ...
    <body>
      ...
      <div class="content">
        ...
        <p>Best regards,<br>The My Stable Prime System</p>
      </div>
    </body>
    </html>
    `;
  ...
  async sendWelcomeEmail(data: WelcomeEmailData): Promise<{ success: boolean; message: string }> {
    return this.sendTemplatedEmail({
      ...
      fallbackSubject: `Welcome to My Stable Prime - ${data.role === 'affiliate' ? 'Affiliate' : 'Admin'} Account Created`,
      variables: data,
  ...
  async sendPasswordResetEmail(email: string, resetToken: string): Promise<{ success: boolean; message: string }> {
    ...
    return this.sendTemplatedEmail({
      ...
      fallbackSubject: 'Password Reset Request - My Stable Prime',
      variables: { resetUrl },
  ...
  async sendVerificationEmail(email: string, verificationToken: string): Promise<{ success: boolean; message: string }> {
    ...
    return this.sendTemplatedEmail({
      ...
      fallbackSubject: 'Verify Your Email Address - My Stable Prime',
    ...
    const html = `
      <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
        <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding: 30px; border-radius: 16px 16px 0 0; text-align: center;">
          <h1 style="color: #ffffff; font-size: 22px; margin: 0;">My Stable Prime Notification</h1>
        </div>
        <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 16px 16px;">
          <p style="color: #374151; font-size: 15px; line-height: 1.6;">${this.escapeHtml(data.body)}</p>
          <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 20px 0;" />
          <p style="color: #9ca3af; font-size: 12px; text-align: center;">This is an automated notification from My Stable Prime.</p>
        </div>
      </div>
    `;
