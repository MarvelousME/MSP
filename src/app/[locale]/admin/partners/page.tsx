      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-teal-500 uppercase tracking-[0.3em]">Directory.Nodes</span>
          </div>
          <h2 className="text-5xl font-black tracking-tighter text-white uppercase italic leading-none">
            Operatives
          </h2>
          <p className="text-slate-500 font-mono text-xs uppercase tracking-widest">
            Managing <span className="text-teal-400">{partners.length}</span> active network nodes
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Button 
            variant="outline" 
            className="rounded-none border-teal-500/20 bg-teal-500/5 hover:bg-teal-500/10 text-teal-400 font-bold uppercase text-[11px] tracking-widest h-12 px-6 transition-all"
          ...
          <Button 
            className="rounded-none bg-orange-600 hover:bg-orange-500 text-white font-black uppercase text-[11px] tracking-widest h-12 px-8 shadow-lg shadow-orange-600/20 transition-all"
            ...
            <TabsList className="bg-transparent h-auto p-0 gap-8">
              {['active', 'pending', 'invited'].map((tab) => (
                <TabsTrigger 
                  key={tab}
                  value={tab} 
                  className="bg-transparent border-none p-0 text-slate-500 data-[state=active]:text-teal-400 font-black uppercase text-[11px] tracking-[0.2em] relative h-10"
                >
                  {tab}
                  {activeTab === tab && (
                    <motion.div 
            ...
            <div className="flex items-center gap-4">
              <div className="relative group">
                ...
                <Input
                  ...
                  className="pl-10 w-72 bg-[#020617]/50 border-white/5 focus:border-teal-500/30 text-xs font-mono tracking-widest text-white rounded-none h-11"
                />
              </div>
            ...
            <Table>
              <TableHeader className="bg-black/20">
                <TableRow className="border-white/5 hover:bg-transparent">
                  ...
                  <TableHead className="cursor-pointer text-[10px] font-black uppercase tracking-[0.2em] text-slate-500" onClick={() => handleSort('name')}>
                    <div className="flex items-center">Operative <SortIcon field="name" /></div>
                  </TableHead>
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Access.ID</TableHead>
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Identity.Stats</TableHead>
                  <TableHead className="cursor-pointer text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-center" onClick={() => handleSort('leads')}>
                    <div className="flex items-center justify-center">Leads <SortIcon field="leads" /></div>
                  </TableHead>
                  <TableHead className="cursor-pointer text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-center" onClick={() => handleSort('customers')}>
                    <div className="flex items-center justify-center">Signals <SortIcon field="customers" /></div>
                  </TableHead>
                  <TableHead className="cursor-pointer text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-right" onClick={() => handleSort('revenue')}>
                    <div className="flex items-center justify-end">Volume <SortIcon field="revenue" /></div>
                  </TableHead>
                  <TableHead className="cursor-pointer text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-right" onClick={() => handleSort('points')}>
                    <div className="flex items-center justify-end">Reputation <SortIcon field="points" /></div>
                  </TableHead>
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-right pr-6">Yield</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredPartners.length > 0 ? (
                  filteredPartners.map((partner, idx) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <div className="flex items-center gap-4">
                          <Avatar className="h-10 w-10 border border-white/10 rounded-none">
                            <AvatarFallback className="bg-slate-900 text-teal-500 text-[10px] font-black rounded-none">
                      ...
                      <TableCell>
                        <code className="text-[10px] font-mono text-orange-500 bg-orange-500/5 px-2 py-1 border border-orange-500/10">
                      ...
                      <TableCell>
                        <div className="flex flex-col">
                          ...
                          <p className="text-[9px] font-mono text-teal-500 uppercase tracking-widest">
                            AGE.{partner.age || '??'}
                          </p>
                        </div>
                      </TableCell>
                      <TableCell className="text-center font-mono text-slate-300 text-xs tracking-tighter">
                  ...
                  <TableRow>
                    <TableCell colSpan={8} className="py-32 text-center">
                      <div className="flex flex-col items-center gap-4 opacity-30">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[0.4em]">Node Registry Empty</p>
                      </div>
                    </TableCell>
                  </TableRow>
                )}
        ...
        <DialogContent className="sm:max-w-xl bg-slate-900 border border-teal-500/20 rounded-none p-0 overflow-hidden text-white shadow-[0_0_80px_rgba(20,184,166,0.1)]">
          ...
          <div className="p-8 space-y-8">
            <DialogHeader className="space-y-2">
              ...
              <DialogDescription className="text-slate-500 font-mono text-[10px] uppercase tracking-widest">Constructing new identity node in the MSP matrix.</DialogDescription>
            </DialogHeader>
            <div className="grid gap-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label className="text-[9px] font-black uppercase text-teal-500/70 tracking-widest ml-1">Identity.First</Label>
                  <Input className="bg-white/5 border-white/10 rounded-none focus:border-teal-500/40 text-xs h-12" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-[9px] font-black uppercase text-teal-500/70 tracking-widest ml-1">Identity.Last</Label>
                  <Input className="bg-white/5 border-white/10 rounded-none focus:border-teal-500/40 text-xs h-12" />
                </div>
              </div>
              <div className="space-y-1.5">
                <Label className="text-[9px] font-black uppercase text-teal-500/70 tracking-widest ml-1">Signal.Address</Label>
                <Input className="bg-white/5 border-white/10 rounded-none focus:border-teal-500/40 text-xs h-12" placeholder="NODE@MSP.SYSTEM" />
              </div>
            </div>
            <DialogFooter className="pt-6 border-t border-white/5 flex gap-3">
              <Button variant="ghost" className="text-slate-500 uppercase text-[10px] font-black tracking-widest h-12" onClick={() => setShowCreateModal(false)}>Cancel.Abort</Button>
              <Button className="bg-teal-600 hover:bg-teal-500 text-white font-black uppercase text-[10px] tracking-widest h-12 flex-1 rounded-none shadow-lg shadow-teal-600/20">Commit Initialization</Button>
            </DialogFooter>
          </div>
        </DialogContent>
      </Dialog>
