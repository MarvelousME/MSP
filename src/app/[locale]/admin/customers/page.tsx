...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  APPROVED: { label: 'NODE.ACTIVE', color: 'emerald' },
  REJECTED: { label: 'NODE.FAILED', color: 'red' },
};
    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-orange-500 uppercase tracking-[0.3em]">Conversion.Stream</span>
          </div>
        ...
        <div className="flex items-center gap-3">
          <Button 
            variant="outline" 
            className="rounded-none border-white/10 bg-white/5 hover:bg-white/10 text-slate-300 font-bold uppercase text-[11px] tracking-widest h-12 px-6"
          >
            Export Lead.Data
          </Button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          { label: 'Converted Nodes', value: stats.approved, icon: UserCheck, className: 'bg-emerald-500/5 border-emerald-500/20 group-hover:bg-emerald-500/10', iconColor: 'text-emerald-500' },
            ...
            <div className="flex gap-4">
              {['all', 'PENDING', 'APPROVED', 'REJECTED'].map((f) => (
                <button
                  ...
                  className={`text-[10px] font-black uppercase tracking-widest transition-colors ${statusFilter === f ? 'text-orange-500' : 'text-slate-600 hover:text-slate-400'}`}
                >
                  {f}
                </button>
              ))}
          ...
          <div className="flex items-center gap-4">
            <div className="relative group">
              ...
              <Input
                ...
                className="pl-10 w-72 bg-[#020617]/50 border-white/5 focus:border-orange-500/30 text-xs font-mono tracking-widest text-white rounded-none h-11"
              />
            </div>
          </div>
        </div>
          ...
          <Table>
            <TableHeader className="bg-black/20">
              <TableRow className="border-white/5 hover:bg-transparent">
                <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 py-5 pl-8">Inbound.Lead</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Origin.Unit</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Source.Partner</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-right">Est.Yield</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-center">Status</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-right pr-8">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filtered.length > 0 ? (
                  ...
                  <TableRow key={referral.id} className="border-white/5 hover:bg-orange-500/[0.02] group transition-all">
                    <TableCell className="py-5 pl-8">
                      <div className="flex items-center gap-4">
                        <Avatar className="h-10 w-10 border border-white/10 rounded-none">
                          <AvatarFallback className="bg-slate-900 text-orange-500 text-[10px] font-black rounded-none">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{referral.company || 'DIRECT.NODE'}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex flex-col">
                        <span className="text-[11px] font-bold text-slate-300 uppercase tracking-tight">{referral.affiliate.name}</span>
                      ...
                      <div className="flex justify-end gap-2">
                        {referral.status === 'PENDING' ? (
                          <>
                            <Button
                              size="sm"
                              className="h-8 rounded-none bg-emerald-600 hover:bg-emerald-500 text-white font-black uppercase text-[9px] tracking-widest px-4"
                          ...
                          <Button
                            variant="ghost"
                            className="h-8 rounded-none text-slate-500 hover:text-white hover:bg-white/5 font-black uppercase text-[9px] tracking-widest px-4"
                ...
                <TableRow>
                  <TableCell colSpan={6} className="py-32 text-center">
                    <div className="flex flex-col items-center gap-4 opacity-30">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-[0.4em]">Signal Spectrum Clear</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
