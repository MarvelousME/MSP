...
export async function GET(
  ...
  try {
    ...
    const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://app.mystableprime.com';
    ...
    // ─── Fraud Detection ───────────────────────────────────────
    const { advancedFraudCheck } = await import('@/lib/fraud');
    const fraudResult = await advancedFraudCheck({
      ipAddress: cleanIP,
      userAgent,
