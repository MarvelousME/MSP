...
export async function GET(request: NextRequest) {
  ...
  try {
    const { races, updatedAt } = await getAggregatedDashboard();
    return jsonWithCors({ updatedAt, races }, origin);
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Failed to load races';
