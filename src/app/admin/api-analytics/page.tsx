    ...
    return (
      <div className="space-y-6">
        <AdminPageHeader
          ...
          eyebrow="API.Metrics"
          title="API Analytics"
          subtitle="Monitor API usage and performance"
        />
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-[15px] text-muted-foreground">in selected period</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <CardContent>
                ...
                <p className="text-[15px] text-muted-foreground">average response time</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <CardContent>
                ...
                <p className="text-[15px] text-muted-foreground">4xx/5xx responses</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <CardContent>
                ...
                <p className="text-[15px] text-muted-foreground">of total requests</p>
              </CardContent>
            </Card>
          </div>
                  ...
                  {data.dailyBreakdown.map((day) => {
                    ...
                    return (
                      <div key={day.date} className="flex-1 flex flex-col items-center gap-1 group" title={`${day.date}: ${day.requests} requests`}>
                        <span className="text-[15px] text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                          {day.requests}
                        </span>
                      ...
                      return (
                        <div key={ep.endpoint} className="space-y-1">
                          <div className="flex items-center justify-between text-sm">
                            <span className="font-mono text-[15px] truncate max-w-[200px]" title={ep.endpoint}>{ep.endpoint}</span>
                            <div className="flex items-center gap-3 text-muted-foreground">
                              <span>{ep.count.toLocaleString()}</span>
                              <span className="text-[15px]">{Math.round(ep.avgResponseMs)}ms</span>
                            </div>
                          </div>
                    ...
                    {data.statusDistribution.map((s) => {
                      ...
                      return (
                        <div key={s.statusCode} className="flex items-center gap-3">
                          ...
                          <span className="text-[15px] text-muted-foreground w-10 text-right">{pct}%</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {data.recentLogs.map((log) => (
                        <TableRow key={log.id}>
                          <TableCell>
                            <span className={`font-mono text-[15px] font-bold ${methodColor(log.method)}`}>{log.method}</span>
                          </TableCell>
                          <TableCell>
                            <span className="font-mono text-[15px] truncate max-w-[200px] block" title={log.endpoint}>{log.endpoint}</span>
                          </TableCell>
                          <TableCell>
                            <Badge variant={statusBadgeVariant(log.statusCode)} className="text-[15px]">{log.statusCode}</Badge>
                          </TableCell>
                          ...
                          <TableCell className="text-[15px] text-muted-foreground font-mono">{log.ipAddress || '—'}</TableCell>
