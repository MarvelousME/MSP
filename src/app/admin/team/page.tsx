...
export default function TeamPage() {
  ...
  const getStatusBadge = (status: string) => (
    <Badge variant={status === 'ACTIVE' ? 'default' : status === 'PENDING' ? 'secondary' : 'destructive'} className="gap-1 text-[15px]">
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        ...
        eyebrow="Crew.Access"
        title="Team Members"
        subtitle="Manage who has access to your admin dashboard"
        right={
                  ...
                  {ROLES.map(role => (
                    <SelectItem key={role.value} value={role.value}>
                      <div>
                        ...
                        <span className="ml-2 text-[15px] text-muted-foreground">{role.description}</span>
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
